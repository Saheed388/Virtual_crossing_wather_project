...
# Define your SAS URL for the append blob
sas_url = "https://saheed338.blob.core.windows.net/historical-weatherd-data/weather_data_yesterday.csv?sp=racwdlme&st=2023-09-14T09:09:09Z&se=2024-09-14T17:09:09Z&sv=2022-11-02&sr=c&sig=9wPtUWoALY8%2Bxc2wh4Lya5GBsrBu6zba7R5YCMuWuJw%3D"

def append_data_to_blob(data, blob_client):
    try:
        blob_client.append_block(data)
        blob_client.flush_append_blob()
        print(f"New data appended to Azure Append Blob: {blob_client.blob_name}")
    except Exception as e:
        print(f"Failed to append new data to Azure Append Blob: {str(e)}")
...
# Append the new data to the Azure Append Blob using the SAS URL
append_data_to_blob(csv_data, blob_client)

print(f"Data for yesterday has been appended to Azure Append Blob")
